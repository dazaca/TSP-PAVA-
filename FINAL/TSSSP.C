...
int ventana1 (void);
void ventana1_2(int TABU[N],int *NN);
...
for(k=0;k<30000;k++)
{
  suma2=dist_revers(TABU,TABU2);
   ...
   if(cont==20)           // una puesta en fondo azul de la pantalla por cada
   ...
   if(*suma1>30000)
   {
	...
	outtextxy(34,427,"T¦");
	rell_rect(54,425,267,437,10,1);
	outtextxy(57,427,gcvt(T, sig,buffer));
	outtextxy(34,442,"áúT¦");
	rell_rect(70,437,267,452,10,1);
...
unsigned int dist_revers(int TABU[N],int TABU2[N])
{
 ...
 int k1,k2,tabukk,i,j,n,pos1,pos2, *P;
 unsigned int suma2;
...
void ventana1_2 (int TABU[N],int *NN)
{
 ...
 {                // porque por defecto todos son cero, n£mero que podr¡a
  TABU[i]=-1;     // coincidir con alguna de la ciudades que se escojan
 }
