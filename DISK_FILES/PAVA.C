...
char nom[]="a:dista.dat"; // Cadena que nos especificar� la ruta del
                          // direcctorio desde el cual se leer� el fichero de
                          // distancias.

int NN,num,control;  // Las variables "NN" y "num" van asociadas al n�mero de
                     // ciudades que ser�n escogidas en cada nuevo tour.
                     // "control" ser� empleada para informar en todo momento
                     // de cu�l debe ser el orden en que se vayan ejecutando
                     // las distintas funciones principales del programa a
                     // trav�s del flujo del mismo.

FILE *fich;   // Variable con la cual podremos acceder al fichero de
              // distancias.

unsigned int suma1; // Es la variable global cuyo valor iremos�
                    // modificando a trav�s de distintas funciones,
                    // informando siempre del kilometraje total del
                    // recorrido que se vaya a seguir.

int TABU[N],TABU2[N]; // Vectores que contendr�n los �ndices de las ciudades
...
// "coord[N][2]" es un array con un n�mero de filas igual al total
// de ciudades candidatas a ser escogidas para realizar un tour.
// Cada �ndice fila se corresponde con el asociado a cada ciudad,
// donde su su correspondiente columna (bien sea la de �ndice 0 o 1) posee las
// coordenadas de la posici�n en la que ubicaremos cada ciudad en pantalla.
...
struct tciudad      // Esta estructura permite asignar a cada �ndice ciudad
{                   // varios par�metros caracter�sticos de �stas, como son
 int posx;          // sus coordenadas tanto horizontal (posx) como vertical
 int posy;          // (posy) en pantalla, el nombre de la ciudad, adem�s de
 char nombre[30];   // la distancia de cada una de �stas a ella misma (0 por
 int distancia[25]; // defecto) y a las dem�s. Todo esto se almacena en
};                  // el vector de estructuras de nombre "ciudad" con un
                    // n�mero de elementos igual al total de ciudades.

struct tciudad ciudad[N];
struct tciudad *pciudad; // Este puntero a estructura se empler� para pasar
...
/// �ste llamado "pciudad" que es el que pasaremos como par�metro en
/// esta funci�n. "num" es una variable global que pasaremos por referencia
/// a la cu�l ir� asociado el n�mero de ciudades totales en nuestra matriz de
/// distancias.
/////////////////////////////////////////////////////////////////////////////
int capt_matr(struct tciudad *pciudad,int *num)
{
 ...
 fscanf(fich,"%d",num); // Capta el n� de ciudades (por defecto almacenado en
			// la primera l�nea del fichero de distancias).

 for(i=0;i<*num;i++)                 // En este bucle se captan los nombres
 {                                   // de las ciudades (por defecto almacenados
  fscanf(fich,"%s",pciudad->nombre); // en la segunda l�nea del fichero de
  pciudad++;                         // distancias).
 }
 ...
 {                                            // Captamos las distancias
  ...
  fscanf(fich,"%d",&(pciudad->distancia[j])); // y a las dem�s, almacen�ndolas en
 ...
 pciudad=posini;  // Una vez m�s, y haciendo uso del puntero "*posini" situamos
 ...
 return 0;  // El valor cero es devuelto por defecto si el fichero de
            ...
            // se ha volcado toda su informaci�n a la estructura "ciudad".


///////////////////////////////////////////////////////////////////////////////
/// Funci�n que recibe como par�metros la matriz "TABU[]" de ciudades escogidas
/// para un tour y su auxiliar "TABU2[]". Tambi�n se le pasa la direcci�n de
/// la variable global "suma" a trav�s de un puntero a la misma. Este paso por
/// valor nos permitir� modificar el valor de la variable mencionada en el
/// el �mbito de esta funci�n. Adem�s de implementarse el algoritmo conocido por
/// el t�rmino ingl�s "Simulated Annealing" para la resoluci�n del problema a
/// a tratar, se efectuan de forma paralela las �rdenes y funciones necesarias
/// para ir visualizando en pantalla la evoluci�n del mismo.
///////////////////////////////////////////////////////////////////////////////
void sim_annel(int TABU[N],int TABU2[N],unsigned int *suma1)
{
  ...
  int sig = 30; // D�gitos significativos empleados en las funciones de conver-
  int sig2=3;   // si�n de valores valores num�ricos a cadenas de caracteres.

  char buffer[300],buffer2[1000],dist_TOUR[7];  // Estas tres cadenas de carac-
  // teres tambi�n ser�n empleadas en las funciones de conversi�n de valores nu-
  // m�ricos a cadenas de caracteres.

  long int diff2_1,cont; // "diff2_1" es la variable donde se almacenar� el
                         // valor de la diferencia de suma2 menos suma1
                         // empleado en la f�rmula del c�lculo de la probabili-
                         // dad.
  long double PROBABILIDAD,T,Tmin,T_FACTOR;
  // "PROBABILIDAD" es aqu� la variable donde se almacenar� la probabilidad
  // que es usada en el algoritmo para "retener" cambios efectuados en los arcos
  // de un recorrido con distancia asociada aparentemente elevada, y que seg�n
  // el valor actual de la soluci�n global (�ntimamente ligado al de aquella)
  // ser� tenido en mayor o menor consideraci�n. Las otras tres variables
  // corresponden a la lista de enfriamiento del algoritmo, donde "T" es la
  // temperatura, "Tmin" la temperatura m�nima y "T_FACTOR" el factor multipli-
  // cativo responsable de la disminuci�n de la temperatura, el cual se corres-
  // ponde con la constante de Boltzmann seg�n la analog�a aplicada al algoritmo
  // con el proceso f�sico del "enfriamiento simulado" empleado en las centrales
  // nucleares.
  T=209200.0;           // Definici�n de las variables que representan los
  T_FACTOR=0.87;        // los par�metros de la lista de enfriamiento.
  Tmin=(3.0*pow10(-5)); //
  cont=0;           // El contador "cont" se emplea para refrescar el contenido
                    // de la pantalla cada cierto n�mero de iteraciones.
 ...
 {
  if (kbhit())tecla=getch(); // Si pulsamos la tecla ESCAPE se abortar�
  if(tecla==27)break;        // el bucle while principal del algoritmo.

  PROBABILIDAD=0.0;
  for(k=0;k<30000;k++) // El valor m�ximo que puede alcanzar "k" corresponde
  {                    // con el n�mero de comprobaciones de distancias alter-
                       // nativas que el algoritmo efectua a base de cambios
		       // por cada arco comprendidos entre dos ciudades
                       // de una ruta preestablecida (s�lo en la primera pasada)
                       // o bien anteriormente modificada por cada variaci�n
                       // que se efectua en "T".

    suma2=dist_revers(TABU,TABU2); // "suma_2" se iguala aqu� al valor de una
                                   // distancia calculada a partir de una ruta
                                   // alternativa que se genera en la funci�n
                                   // "dist_revers".

    if(suma2<=*suma1)  // Se compara la mejor soluci�n obtenida hasta el momento
    {                  // (suma1) con la proporcionada por la funci�n
     *suma1=suma2;     // "dist_revers" ("suma_2"). En caso de ser esta �ltima
    ...
    else    // En este bloque se efect�an posibles captaciones de
    {       // distancias con recorridos aparentemente "malos" (distancias
            // grandes), en funci�n del estado actual de la probabilidad.

     diff2_1=suma2-*suma1;

     PROBABILIDAD=exp(-(diff2_1)/T); // F�rmula para el c�lculo de la probabilidad.
   ...
   cont++;             // Para evitar un exceso de informaci�n en pantalla, se
   ...
   if(*suma1>30000)
   {
    ...
    setcolor(4);      // Puesta en pantalla de los par�metros de la lista de
    ...
    if(PROBABILIDAD!=0)  // El valor de la probabilidad se imprimir� en pantalla
    {                    // s�lo cuando sea distinto de cero.
   ...
   setcolor(12);
   traz_lineas(); // LLamamos a la funci�n "traz_lineas" que nos permite visua-
		  // lizar en pantalla el recorrido de un tour por las ciudades
		  // seleccionadas.
   if(kbhit())     // Esta condici�n se empleaa en caso de pulsar la tecla
   {               // INTRO; de esta forma el flujo del programa se detendr�
    ...
    if(tecla==27)break; // En caso de pulsar ESC, bien desde el punto de parada
			...
			// est� iterando, se producir� una ruptura en del
			// bucle while y se producir� una interupci�n autom�tica-
 }//FIN WHILE           // mente en �ste.
   ...
   setbkcolor(BLUE);       // Se hace un �ltimo trazado de la mejor ruta encon-
   traz_lineas();          // trada una vez finalizada la b�squeda de distancias
   marc_ciudades_selecc(); // tras una puesta en azul de la pantalla. La funci�n
   setcolor(14);           // "marc_ciudades_selecc()" nos permitir� ver los nom-
			   // bres de las ciudades que se han escogido para el
			   // recorrido, aparte de localizarlas en pantalla con
			   // un peque�o c�rculo de centro la coordena asociada
   ...
   outtextxy(30,387,"Distancia Total"); // par�metros de la lista de enfriamien-
   ...
   rell_rect(303,437,621,452,2,1);    // Por �ltimo se imprime el valor de la
   outtextxy(305,442,"Probabilidad:");// probabilidad (siempre cero al final de cada
   rell_rect(410,439,609,450,10,1);   // evaluaci�n por defecto).
   outtextxy(413,441,gcvt(PROBABILIDAD, sig,buffer2));
   fflush(stdout);
...
/////////////////////////////////////////////////////////////////////////////
/// Esta es la funci�n empleada dentro de "sim_annel()" que permite efectuar
...
unsigned int dist_revers(int TABU[N],int TABU2[N])
{
 ...
 unsigned int suma2;
 suma2=0;
 do                       // En este bucle se procede a la b�squeda del primer �ndice
 {                        // v�lido de ciudades que est�n en el recorrido (por
   pos1=0;                // tanto est�n en "TABU[]") con el cual se efectuar�
   tabukk=0;              // un cambio de arco. En caso de no encontarlo, la
   k1=random(N);          // variable "tabukk" se autoincrementa inform�ndonos
			  // con ello posteriormente de que dicho �ndice no es
   for(n=0; n<=(NN-1);n++)// apto para efectuar el cambio local en el recorrido
   {                      // total dado por todos los �ndices de "TABU[]".
    if (k1==TABU[n])
    {
     tabukk++;
     break;
 ...
 }while(!tabukk);         // nueva distancia. S�lo se autoincrementa si se ha
			  // encontrado un �ndice v�lido ("tabukk"=0).
 do
 {
  pos2=0;
  tabukk=0;
 ...
 *(TABU2+pos2)=k1;// los valores "k2" y "k1" respectivamente que en TABU
 ...
 i=*(TABU2+(NN-1));   // Aqu� termina el c�lculo de un tour completo a�adi�ndose
 j=*TABU2;            // la distancia del arco de cierre del tour dado por la
 suma2+=(unsigned int)ciudad[i].distancia[j];  // ciudad de origen y la �ltima
 ...
 *(P+n)=24;           // Hemos de inicializar todos los elementos de TABU[]
		      ...
		      // podr�a provocar fallos en el programa si el usuario
		      // decidiera ejecutar m�s de una vez la opci�n de un tour
 ...
 do
 {
   ...
   *(TABU+(n+1))=k;   //Y lo va introduciendo en "TABU[]" hasta que �ste se llene.
   n++;
 }while(n<N-1);
 *NN=N;
}
...
void inicia_graf()
{
 ...
 errorcode = graphresult();   // Lee el resultado de la inicializaci�n.
 if (errorcode != grOk)       // En caso de error termina el programa
 {                            // emitiendo antes un c�digo de error.
...
{
 int i,j;
 settextstyle(0,0,1);  // Aqu� se hace uso de la informaci�n relativa a cada
 j=TABU[0];            // ciudad almacenada en la estructura "ciudad"; concr�-
...
/// las ciudades del recorrido calculado para unirlas a trav�s de l�neas
...
void traz_lineas(void)
{
 ...
 setcolor(12); // Impresi�n de la ciudad de partida en color rojo suavizado.
 outtextxy(ciudad[i].posx+5,ciudad[i].posy-3,ciudad[i].nombre);
 ...
 //Unimos el recorrido con sucesivos arcos dados por el orden de colocaci�n de
 // las ciudades cuyos �ndices asociados se almacenaron en "TABU[]".
 for(j=0;j<NN;j++)
 {
  i=TABU[j];
...
int ventana1 (void)
{
 ...
 for(;;)                          // Si la tecla pulsada es 1 la funci�n devolver�
 {                                // este mismo valor, que luego emplearemos para
  ...
  {                               // del programa seg�n sea. Idem si la tecla
...
/// posible seleccionar aquellas ciudades que deseamos est�n en nuestra ruta.
/////////////////////////////////////////////////////////////////////////////
int ventana1_2 (int TABU[N],int *NN)
{
 ...
 {                // porque por defecto todos son cero, n�mero que ser�a
  TABU[i]=-1;     // reconocido como el �ndice asociado a la ciudad 1.
 }
 ...
 j=0;// El contador j permite seguir un orden unitario creciente para as�
     ...
     // a los nombres que ser�n colocados en pantalla en una segunda columna.
 ...
 rell_rect(285,410,309,428,0,1); // En este punto se introduce una peque�a
				 // ventana donde se ir�n visualizando los
 ...
 *NN=0;
 city=1;
 j=0;
 do // Este bucle permite introducir d�gitos de valor 1 a N que corresponder
 {  // con los �ndices asociados a las ciudades que el usuario seleccione.
  ...
  {       // v�lido.
   ...
   if((tecla==8)&&(i>0))
   {
    ...
    rell_rect(285,410,309,428,0,1);// con un relleno negro de rect�ngulo toda el
  ...
  if(tecla==27)break;  // La opci�n de abandonar el men� que se genera en esta
		       // funci�n es posible gracias a esta condici�n, que tras
		       // producir una ruptura en la ejecuci�n de la misma tam-
		       // bi�n ser� responsable de modificar el valor que devuelva
  ...
  city=atoi(cadena); // Los d�gitos tecleados que han sido almacenados en
		     ...
		     // evaluaci�n como posible �ndice v�lido asociado a una ciudad
  ...
  {                         // ducir en "TABU[]" �ndices v�lidos que nos marquen
   ...
   for(i=0;i<N;i++)
   {
    if(TABU[i]==city-1)   // Comprueba que el �ndice de la ciudad no est� ya
    {                     // contenido en TABU[]. En caso de que lo encuentre
     tabu++;              // la variable centinela "tabu" se pone a uno.
    }
   }

   if(tabu==0)  // Si el valor del �ndice v�lido no se encontr� se procede a
   {            // su introducci�n en TABU[].
    ...
    TABU[*NN]=city-1; // N�tese aqu� que si p.e. tecleamos el valor 1 como
		      ...
		      // nado en TABU[] ser� 0, ya que este es el verdadero
		      // �ndice que dada la notaci�n matricial se asigna al
...
/////////////////////////////////////////////////////////////////////////////
///  Esta funci�n corresponde con la �ltima ventana en la cual se ofrece la
...
/////////////////////////////////////////////////////////////////////////////
/// Ventana que muestra recuadrado el t�tulo de la la introducci�n al programa.
/////////////////////////////////////////////////////////////////////////////
void intro(void)
{
 fflush(stdout);
...
void main(void)
{
 ...
 for(;;)
 {
  switch (control)    // "control" determina el flujo del programa nutri�ndose
